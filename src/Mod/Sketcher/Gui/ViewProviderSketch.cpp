...
void ViewProviderSketch::slotRedoDocument(const Gui::Document& /*doc*/)
{
    ...
    // the next event callback so relax-dimension hover/pick never touches pre-redo GeoIds.
    cancelSmartDimensionInteraction();
    smartDimensionPreviewRefreshPending = false;
    clearDimensionCandidates();
                                            ...
                                            const Gui::View3DInventorViewer* viewer)
{
    ...
    if (Button == 1) {
        if (pressed) {
            ...
            // even though the relax-dimension interaction is still the active gesture. When that
...
void ViewProviderSketch::onSelectionChanged(const Gui::SelectionChanges& msg)
{
    ...
    if (isInEditMode()) {
        ...
        if (msg.Type == Gui::SelectionChanges::ClrSelection
            ...
            // Clicking a relax-dimension preview can temporarily raise selection updates from the
...
bool ViewProviderSketch::beginSmartDimensionInteraction(const QPoint& screenPos)
{
    ...
    // A real sketch point must win over the relax-dimension overlay. When a curve is already
