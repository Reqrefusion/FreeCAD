...
bool isLazyExternalSketchConstraintSubElementName(const char* subName)
{
    ...
    return element.rfind("Edge", 0) == 0 || element.rfind("Vertex", 0) == 0;
}

bool getLazyExternalSelectionObject(SketcherGui::ViewProviderSketch* sketchgui,
                                    ...
                                    App::DocumentObject*& selectedObject)
{
    ...
    if (!sketchDocument || std::string(msg.pDocName) != sketchDocument->getName()) {
        return false;
    }

    selectedObject = sketchDocument->getObject(msg.pObjectName);
    if (!selectedObject) {
        std::string normalizedObjectName(msg.pObjectName);
...
enum class LazyExternalPreselectionState
{
    None,
    UsableExternal,
    IgnoredExternal
...
std::map<SketcherGui::ViewProviderSketch*, SbVec2s> lazyExternalIgnoredPressCandidates;
std::map<SketcherGui::ViewProviderSketch*, SbVec2s> lazyExternalUsablePressCandidates;
...
void setLazyExternalIgnoredPreselection(SketcherGui::ViewProviderSketch* sketchgui, bool active)
{
    ...
    if (active) {
        ignoredLazyExternalPreselectionSketches.insert(sketchgui);
    }
    else {
        ignoredLazyExternalPreselectionSketches.erase(sketchgui);
    }
}

bool consumeLazyExternalIgnoredPreselection(SketcherGui::ViewProviderSketch* sketchgui)
{
    if (!sketchgui) {
        return false;
    }

    auto it = ignoredLazyExternalPreselectionSketches.find(sketchgui);
    if (it == ignoredLazyExternalPreselectionSketches.end()) {
        return false;
    }
...
bool getLazyExternalPreselectionSnapshotFromPickedPoint(
    SketcherGui::ViewProviderSketch* sketchgui,
    ...
    LazyExternalPreselectionSnapshot& snapshot)
{
    ...
    if (selectedObject == sketchObject) {
        // This is the active sketch itself.  Let the normal Sketcher empty-space
        // and sketch-geometry paths decide whether to start rubber-band selection.
        return false;
    }

    std::string pickedSubName;
    if (!viewProviderObject->getElementPicked(pickedPoint, pickedSubName)) {
        pickedSubName.clear();
    }

    snapshot.documentName = selectedObject->getDocument()
        ? selectedObject->getDocument()->getName()
        : std::string();
    snapshot.objectName = selectedObject->getNameInDocument();
    snapshot.subName = pickedSubName;
    snapshot.cursorPos = cursorPos;
    snapshot.hasCursorPos = true;

    const bool usableSubElement = isLazyExternalSketchConstraintSubElementName(snapshot.subName.c_str());
    if (usableSubElement
        ...
        // Any non-edge/non-vertex external pick, especially faces, must not fall
        // through to Sketcher's empty-space branch.  Empty-space starts rubber-band
        // and clears prior selections on release; an external face should be ignored.
...
void setLazyExternalIgnoredPressCandidate(SketcherGui::ViewProviderSketch* sketchgui,
                                          const SbVec2s& pressPos)
{
    ...
    lazyExternalIgnoredPressCandidates[sketchgui] = pressPos;
}

bool hasLazyExternalIgnoredPressCandidate(SketcherGui::ViewProviderSketch* sketchgui)
{
    return sketchgui && lazyExternalIgnoredPressCandidates.find(sketchgui)
        != lazyExternalIgnoredPressCandidates.end();
}

bool consumeLazyExternalIgnoredPressCandidate(SketcherGui::ViewProviderSketch* sketchgui)
{
    if (!sketchgui) {
        return false;
    }

    auto it = lazyExternalIgnoredPressCandidates.find(sketchgui);
    if (it == lazyExternalIgnoredPressCandidates.end()) {
        return false;
    }

    lazyExternalIgnoredPressCandidates.erase(it);
    return true;
}

void clearLazyExternalIgnoredPressCandidate(SketcherGui::ViewProviderSketch* sketchgui)
{
    if (!sketchgui) {
        return;
    }

    lazyExternalIgnoredPressCandidates.erase(sketchgui);
}

void setLazyExternalUsablePressCandidate(SketcherGui::ViewProviderSketch* sketchgui,
                                         const SbVec2s& pressPos)
{
    if (!sketchgui) {
        return;
    ...
    lazyExternalUsablePressCandidates[sketchgui] = pressPos;
}

bool consumeLazyExternalUsablePressCandidate(SketcherGui::ViewProviderSketch* sketchgui)
{
    if (!sketchgui) {
        return false;
    }

    auto it = lazyExternalUsablePressCandidates.find(sketchgui);
    if (it == lazyExternalUsablePressCandidates.end()) {
        return false;
    }

    lazyExternalUsablePressCandidates.erase(it);
    return true;
}

void clearLazyExternalUsablePressCandidate(SketcherGui::ViewProviderSketch* sketchgui)
{
    if (!sketchgui) {
        return;
    }

    lazyExternalUsablePressCandidates.erase(sketchgui);
}
...
struct LazyExternalSelectionItem
{
    std::string documentName;
    std::string objectName;
    std::string subName;
...
std::vector<LazyExternalSelectionItem> collectLazyExternalSelectionItemsToPreserve(
    SketcherGui::ViewProviderSketch* sketchgui)
{
    std::vector<LazyExternalSelectionItem> items;
    if (!sketchgui || !sketchgui->getSketchObject()) {
        return items;
    }

    Sketcher::SketchObject* sketchObject = sketchgui->getSketchObject();
    const char* sketchDocName = sketchObject->getDocument()->getName();
    const char* sketchObjectName = sketchObject->getNameInDocument();

    std::vector<Gui::SelectionObject> selections = Gui::Selection().getSelectionEx();
    for (Gui::SelectionObject& selectionObject : selections) {
        App::DocumentObject* selectedObject = selectionObject.getObject();
        if (!selectedObject || !selectedObject->getDocument()) {
            continue;
        }

        const bool activeSketchSelection = selectedObject == sketchObject;
        const bool lazyExternalSelection = selectedObject != sketchObject
            && sketchObject->isExternalAllowed(selectedObject->getDocument(), selectedObject);
        if (!activeSketchSelection && !lazyExternalSelection) {
            continue;
        }

        const char* docName = activeSketchSelection
            ? sketchDocName
            : selectedObject->getDocument()->getName();
        const char* objectName = activeSketchSelection
            ? sketchObjectName
            : selectedObject->getNameInDocument();

        for (const std::string& subName : selectionObject.getSubNames()) {
            if (subName.empty()) {
                continue;
            }
            if (!activeSketchSelection
                && !isLazyExternalSketchConstraintSubElementName(subName.c_str())) {
                continue;
            }

            items.push_back({docName, objectName, subName});
        }
    }
...
void restoreLazyExternalSelectionItems(
    const std::vector<LazyExternalSelectionItem>& items,
    const LazyExternalSelectionItem& toggledItem,
    bool toggledOff)
{
    for (const LazyExternalSelectionItem& item : items) {
        if (toggledOff
            && item.documentName == toggledItem.documentName
            && item.objectName == toggledItem.objectName
            && item.subName == toggledItem.subName) {
            continue;
        }

        if (!Gui::Selection().isSelected(item.documentName.c_str(),
                                         item.objectName.c_str(),
                                         item.subName.c_str())) {
            Gui::Selection().addSelection(item.documentName.c_str(),
                                          item.objectName.c_str(),
                                          item.subName.c_str());
        }
    }
...
{
    // NOTE: Do NOT call rmvPreselect() or rmvSelection() from inside a SelectionChanges
    // notification handler.  Those calls re-enter onSelectionChanged() synchronously,
    // which can clear Sketcher's own hover state and make the next sketch-curve hover
    // require an extra click.  For SetPreselect events the snapshot mechanism already
    // records the ignored face so mouseButtonPressed() can treat it as empty space.
    // For AddSelection events on faces/whole-objects the gate allows them through (to
    // avoid the red forbidden cursor), but the handler simply ignores them; clearing them
    // here would generate a spurious ClrSelection or RmvSelection that resets the ongoing
    // constraint sequence.
    (void)msg;
}
                                       ...
                                       const Gui::SelectionChanges& msg)
{
    ...
    if (!usableSubElement || !sketchObject->isExternalAllowed(selectedObject->getDocument(), selectedObject)) {
        ...
        // AddSelection for a face/whole-object: consume silently.
        // discardLazyExternalSelectionChange() is intentionally a no-op -- calling
        // rmvSelection() from inside a selection-change handler creates reentrant events
        // that poison Sketcher hover state.  The stale entry is harmless and will be
        // swept away on the next cursor move or ClrSelection.
    ...
    // Keep valid external Edge/Vertex AddSelection events flowing through so the active
    // constraint handler can see them and trigger materialization.
    // Consume RmvSelection after snapshot has been cleared above.
...
void ViewProviderSketch::deactivateHandler()
{
    ...
    // With no active draw/constraint handler, lazy external edges should again be hoverable
    // and selectable in the 3D view.  This mirrors v11's useful behaviour without letting
    // native selection interfere with drawing tools.
                                            ...
                                            const Gui::View3DInventorViewer* viewer)
{
    ...
    if (Button == 1) {
        if (pressed) {
            ...
            switch (Mode) {
                case STATUS_NONE: {
                    ...
                    if (!done) {
                        ...
                        auto lazyExternalPreselection =
                            classifyLazyExternalPreselection(this, cursorPos, lazyExternalSnapshot);
                        if (lazyExternalPreselection == LazyExternalPreselectionState::None
                            && getLazyExternalPreselectionSnapshotFromPickedPoint(
                                this,
                                pp.get(),
                                cursorPos,
                                lazyExternalSnapshot)) {
                            lazyExternalPreselection = lazyExternalSnapshot.state;
                        }
                        if (lazyExternalPreselection
                            == LazyExternalPreselectionState::UsableExternal) {
                            // Select the exact external Edge/Vertex that is currently under the
                            // cursor.  The picked-point fallback covers the case where native
                            // preselection has not been emitted before mouse-down.
                            ...
                            clearLazyExternalIgnoredPressCandidate(this);
                            clearLazyExternalFaceRubberBandCandidate(this);
                            setLazyExternalUsablePressCandidate(this, cursorPos);
                            return selectLazyExternalPreselection(this, lazyExternalSnapshot, x, y);
                        }
                        if (lazyExternalPreselection
                            == LazyExternalPreselectionState::IgnoredExternal) {
                            // A face/whole-object hit is not an immediate rubber-band start.
                            // Treat it as a pending empty-space click: a click clears selection
                            // on release, while a real drag starts rubber-band after the normal
                            // Qt drag threshold.  This keeps face clicks from exploding into
                            // rubber-band, but still lets users box-select by dragging on a face.
                            DoubleClick::prvClickTime = SbTime::getTimeOfDay();
                            DoubleClick::prvClickPos = cursorPos;
                            DoubleClick::prvCursorPos = cursorPos;
                            DoubleClick::newCursorPos = cursorPos;
                            clearLazyExternalIgnoredPressCandidate(this);
                            clearLazyExternalUsablePressCandidate(this);
            ...
            if (consumeLazyExternalUsablePressCandidate(this)) {
                // Mouse-down already toggled a lazy external Edge/Vertex.  Consume the
                // matching mouse-up so it cannot be interpreted as a blank click or keep
                // the source-object preselection alive for the next click.
                Gui::Selection().rmvPreselect();
                clearLazyExternalPreselectionSnapshot(this);
                setSketchMode(STATUS_NONE);
                return true;
            }

            if (consumeLazyExternalIgnoredPressCandidate(this)) {
                // The press started on an ignored external face/whole-object hit.
                // Keep existing selections intact and do not let the release become a
                // late native face selection.
                clearLazyExternalFaceRubberBandCandidate(this);
                setSketchMode(STATUS_NONE);
                setLazyExternalSketchNativeSelection(this, true);
                return true;
            ...
            if (consumeLazyExternalFaceRubberBandCandidate(this)) {
                // Face click without enough drag: behave like a normal empty-space click
                // for selection state, but never select the face/whole object itself.
...
bool ViewProviderSketch::mouseMove(const SbVec2s& cursorPos, Gui::View3DInventorViewer* viewer)
{
    ...
    if (hasLazyExternalIgnoredPressCandidate(this)) {
        // Movement while the mouse is down over an ignored external face must not
        // promote to rubber-band.  The matching release will consume the click.
        return true;
    }
    ...
    if (getLazyExternalFaceRubberBandCandidateStart(this, lazyExternalFaceRubberBandStart)) {
        // The press began over an ignored external face.  Mirror the stock Sketcher
        // empty-space path: a tiny motion is ignored, and crossing the same small
        // dragIgnoredDistance threshold promotes the gesture to rubber-band selection.
        short dx = 0;
        short dy = 0;
...
void ViewProviderSketch::onSelectionChanged(const Gui::SelectionChanges& msg)
{
    ...
    if (isInEditMode()) {
        ...
        if (handleLazyExternalSelectionChange(this, msg)) {
            ...
            // Non-SetPreselect events that are consumed here (e.g. IgnoredExternal
            // AddSelection) must NOT reach the handler.
            if (Mode == STATUS_SKETCH_UseHandler
...
void ViewProviderSketch::setEditViewer(Gui::View3DInventorViewer* viewer, int ModNum)
{
    ...
    // Keep native selection available while the sketch is idle so lazy external Edge/Vertex
    // preselection works. activateHandler() disables it for drawing tools, and constraint
    // handlers re-enable it only for their own external-pick workflow.
    viewer->setSelectionEnabled(true);
