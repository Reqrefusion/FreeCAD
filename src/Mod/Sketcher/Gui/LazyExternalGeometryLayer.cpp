...
double distancePointToSegment2d(const Base::Vector2d& p,
                                ...
                                std::vector<Base::Vector3d>& curveCoords)
{
    ...
    const double tolerance = std::max(Precision::Confusion(),
                                      chordLength * DisplayCurveRelativeDeviation);
    const double deviation = distancePointToSegment2d(toVector2d(middlePoint),
    ...
    appendAdaptiveCurveSegment(curve,
                               first,
                               middle,
                               firstPoint,
                               middlePoint,
...
void appendCurveToCoin(const Part::Geometry* geometry,
                       std::vector<Base::Vector3d>& curveCoords,
                                        ...
                                        bool intersection)
{
    ...
    for (std::size_t i = 0; i < objects.size() && i < subElements.size(); ++i) {
        ...
        ExternalGeometryMatch match{static_cast<int>(i),
                                    matchesExternalType(types[i], intersection)};
