                                        ...
                                        Base::Vector2d& position)
{
    ...
    const auto projectPoint = [](const Base::Vector3d& point) { return Base::Vector2d(point.x, point.y); };
    ...
    switch (constraint->Type) {
        ...
        case Sketcher::Distance:
            if (secondPoint.y < firstPoint.y - eps
                || (std::abs(secondPoint.y - firstPoint.y) <= eps && secondPoint.x < firstPoint.x - eps)) {
                std::swap(firstPoint, secondPoint);
            }
            {
    ...
    if (doc && doc->getInEdit()
        ...
        for (int i = lastConstraintIndex; i >= firstConstraintIndex; i--) {
            if (lastConstraintType == Radius || lastConstraintType == Diameter) {
                ...
                Base::Vector2d linearLabelPosition;
                if (LinearDatumLabelPlacement::computeLabelPosition(sketch,
                                                                    ConStr[i],
                                                                    linearLabelPosition)) {
                    ViewProviderSketchCommandConstraintsAttorney::moveConstraint(*vp,
                                                                                i,
                                                                                linearLabelPosition);
                }
            }
