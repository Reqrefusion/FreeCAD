...
{
    // Keep the first implementation intentionally narrow: one selected edge or vertex is
    // projected as one Sketcher external geometry.  Faces can expand to several geometries and
    // would violate the "only the referenced one" rule.
    return startsWith(subName, "Edge") || startsWith(subName, "Vertex");
}
    ...
    const char* subName)
{
    ...
    int geoId = Obj->findExternalGeometry(selObj, subName);
    ...
    geoId = Obj->findExternalGeometry(selObj, subName);
    if (geoId != Sketcher::GeoEnum::GeoUndef) {
        return geoId;
    }
...
static bool isExternalConstraintPreselection(Sketcher::SketchObject* Obj)
{
    if (!Obj) {
        return false;
    }
    ...
    if (Base::Tools::isNullOrEmpty(preselection.pObjectName)) {
        return false;
    }

    App::DocumentObject* selObj = getLazyExternalConstraintObject(
        Obj->getDocument(),
        preselection.pDocName,
        preselection.pObjectName);

    return selObj && selObj != Obj;
}

static bool resolveLazyExternalConstraintPreselection(
    Gui::Command* cmd,
    Sketcher::SketchObject* Obj,
    unsigned int allowedSelTypes,
    SelIdPair& selIdPair,
    ...
    std::stringstream& selectionName)
{
    ...
    const auto& preselection = Gui::Selection().getPreselection();
    if (Base::Tools::isNullOrEmpty(preselection.pObjectName)
        || Base::Tools::isNullOrEmpty(preselection.pSubName)) {
        return false;
    }
...
public:
    ...
    bool onSelectionChanged(const Gui::SelectionChanges& msg) override
    {
        ...
        if (msg.Type != Gui::SelectionChanges::AddSelection
            || Base::Tools::isNullOrEmpty(msg.pSubName)) {
            return false;
        }
        ...
        if (selectedObject == sketchObject) {
            // Normal sketch selections must continue to reach ViewProviderSketch so the
            // selected/preselected visual state is updated exactly like upstream Sketcher.
            // The constraint handler still performs the logical pick in releaseButton();
            // duplicate AddSelection notifications are harmless, while swallowing this event
            // leaves the clicked edge looking merely preselected.
            return false;
        }
...
protected:
    ...
    bool cachedExternalPreselectionValid = false;
    std::string cachedExternalDocumentName;
    std::string cachedExternalObjectName;
    std::string cachedExternalSubName;
    ...
    void clearCachedExternalPreselection()
    {
        cachedExternalPreselectionValid = false;
        cachedExternalDocumentName.clear();
        cachedExternalObjectName.clear();
        cachedExternalSubName.clear();
    }

    bool cacheExternalPreselection(const Gui::SelectionChanges& msg)
    {
        clearCachedExternalPreselection();

        Sketcher::SketchObject* sketchObject = sketchgui ? sketchgui->getSketchObject() : nullptr;
        if (!sketchObject || Base::Tools::isNullOrEmpty(msg.pSubName)
            || !isLazyExternalConstraintSubElement(msg.pSubName)) {
            return false;
        }
        ...
        cachedExternalDocumentName = Base::Tools::isNullOrEmpty(msg.pDocName)
            ? std::string()
            : std::string(msg.pDocName);
        cachedExternalObjectName = msg.pObjectName;
        cachedExternalSubName = msg.pSubName;
        cachedExternalPreselectionValid = true;
        return true;
    }

    bool isCurrentLazyExternalConstraintPreselection() const
    {
        Sketcher::SketchObject* sketchObject = sketchgui ? sketchgui->getSketchObject() : nullptr;
        if (!sketchObject) {
            return false;
        ...
        if (Base::Tools::isNullOrEmpty(preselection.pObjectName)
            || Base::Tools::isNullOrEmpty(preselection.pSubName)
            || !isLazyExternalConstraintSubElement(preselection.pSubName)) {
            return false;
        }
    ...
    bool resolveCachedExternalPreselection(SelIdPair& selIdPair,
                                           SelType& newSelType,
                                           std::stringstream& selectionName)
    {
        Sketcher::SketchObject* sketchObject = sketchgui ? sketchgui->getSketchObject() : nullptr;
        if (!sketchObject || !cachedExternalPreselectionValid) {
            return false;
        }

        App::DocumentObject* selectedObject = getLazyExternalConstraintObject(
            sketchObject->getDocument(),
            cachedExternalDocumentName.empty() ? nullptr : cachedExternalDocumentName.c_str(),
            cachedExternalObjectName.c_str());
        if (!selectedObject || selectedObject == sketchObject) {
            clearCachedExternalPreselection();
            return false;
        }
    ...
    bool consumeExternalPreselection(Base::Vector2d onSketchPos)
    {
        ...
        // Not-yet-added lazy external geometry is preselected on its source object.
