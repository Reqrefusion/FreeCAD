...
using SketcherGui::CommandConstraintsInternal::finishDatumConstraint;
...
bool SketcherGui::prepareDimensionDatumPlacement(
...
)
{
    if (constraint.Type == Sketcher::Distance || constraint.Type == Sketcher::DistanceX
        ...
        if (SketcherGui::LinearDatumLabelPlacement::computeLabelPosition(&sketch,
                                                                         &constraint,
                                                                         labelPosition)) {
            return prepareDimensionDatumPlacement(sketch, constraint, labelPosition);
        }
    }
...
void SketcherGui::CommandConstraintsInternal::finishDatumConstraint(
    Gui::Command* cmd,
    Sketcher::SketchObject* sketch,
    bool isDriving,
    unsigned int numberofconstraints,
    std::optional<Base::Vector2d> datumPlacement,
    SketcherGui::CommandConstraintsInternal::DatumPlacementMode placementMode
)
{
    ...
    if (doc && doc->getInEdit()
        ...
        for (int i = lastConstraintIndex; i >= firstConstraintIndex; i--) {
            if (
                placementMode
                == SketcherGui::CommandConstraintsInternal::DatumPlacementMode::Preserve
            ) {
                continue;
            }
...
bool SketcherGui::CommandConstraintsInternal::shouldCreateDrivingDimension(
    const Sketcher::SketchObject* sketch,
    int geoId1,
    int geoId2
)
