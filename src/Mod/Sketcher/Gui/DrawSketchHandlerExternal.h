...
public:
    ...
    bool onSelectionChanged(const Gui::SelectionChanges& msg) override
    {
        if (msg.Type == Gui::SelectionChanges::AddSelection) {
            ...
            const bool isExternalEdge = hasValidIndex
                && std::strcmp(subName.getType(), "Edge") == 0;
            const bool isExternalVertex = hasValidIndex
                && std::strcmp(subName.getType(), "Vertex") == 0;
            const bool isExternalFace = hasValidIndex
                && std::strcmp(subName.getType(), "Face") == 0;
...
private:
    ...
    void suspendLazyExternalLayer()
    {
        if (!lazyLayerSuspended && sketchgui) {
            sketchgui->suspendLazyExternalGeometryLayer();
    ...
    {
        if (lazyLayerSuspended && sketchgui) {
            sketchgui->resumeLazyExternalGeometryLayer();
